...
def generate_page(tables_list: list):
	...
	output_start += (
		'<button class="button after" type="button" '
		'onClick="show_all()">Kaikki</button>\n'
	)
	...
	for x, v in enumerate(tables_list):
		...
		output_start += (
			f'<button class="button after" type="button" '
			f'onClick="show_only({x})">{provider}</button>\n'
		)
	...
	script_to_add = """
		function show_only(index) {
			const wantedId = `taulu${index}`;

			document.querySelectorAll("table.taulu").forEach((table) => {
				table.style.display =
					table.id === wantedId ? "" : "none";
			});
		}
